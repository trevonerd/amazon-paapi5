 ...
 *
 */
'use strict'; // sources of inspiration:
...
exports.createAuthorizationHeaders = function (timestamp, accessKey, region, service, signedHeaders, signature) {
  return 'AWS4-HMAC-SHA256' + ' ' + 'Credential=' + accessKey + '/' + exports.createCredentialScope(timestamp, region, service) + ', ' + 'SignedHeaders=' + signedHeaders + ', ' + 'Signature=' + signature;
};

exports.createCanonicalRequest = function (method, pathname, query, headers, payload) {
  var payloadJson = JSON.stringify(payload);
  return [method.toUpperCase(), pathname, exports.createCanonicalQueryString(query), exports.createCanonicalHeaders(headers), exports.createSignedHeaders(headers), hexEncodedHash(String(payloadJson))].join('\n');
};
...
exports.createStringToSign = function (time, region, service, request) {
  return ['AWS4-HMAC-SHA256', toTime(time), exports.createCredentialScope(time, region, service), hexEncodedHash(request)].join('\n');
};
